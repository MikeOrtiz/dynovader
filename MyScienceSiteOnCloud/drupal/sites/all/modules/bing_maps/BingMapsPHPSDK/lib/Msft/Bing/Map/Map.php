...
class Msft_Bing_Map
{
	...
	private $bingLocation = 'bing_node_location';
	...
	{
		echo 'InitializeMap(\''.$divName.'\', '.$this->properties->getzoomLevel().', '.$this->properties->getWidth().', '.$this->properties->getHeight().', \''.$this->properties->getMapType().'\', \''.$this->properties->getDefaultPosition().'\');';
	}
	...
	private function locateLibPath()
	{
		...
		$libPath = $path.'/BingMapsPHPSDK/lib';
	...
	public function createConfigurationEntity()
	{
		...
		if ($result == 0)
		{
			...
			$col->AllowNull = true;
		  	...
		  	$col = new Msft_Bing_Column('position');
			$col->DataType = 'varchar';
			$col->MaxLength = 200;
			$col->AllowNull = true;
		  	$cols[] = $col;	
	...
	public function saveConfiguration()
	{
		...
		$pinsArray[0]->position = '\''.$mapPro->getDefaultPosition().'\'';		
		...
		if ($result!=null && $result!='')
	  	{
		  	...
		  	{
		  		$pinResConfig[$pr] = new stdClass();
		  		$pinResConfig[$pr]->slno = $result[$pr]->Data['slno'];
		  		$pinResConfig[$pr]->bingKey = $result[$pr]->Data['bing_key'];
		  		
		  		$pinResConfig[$pr]->positionID = $result[$pr]->Data['positionid'];
		  		
		  		$pinResConfig[$pr]->width = $result[$pr]->Data['width'];
		  		$pinResConfig[$pr]->height = $result[$pr]->Data['height'];
		  		$pinResConfig[$pr]->location = $result[$pr]->Data['location'];
		  		$pinResConfig[$pr]->zoomLevel = $result[$pr]->Data['zoomlevel'];
		  		$pinResConfig[$pr]->content_type = $result[$pr]->Data['content_type'];
		  		$pinResConfig[$pr]->defaultPosition = $result[$pr]->Data['position'];
		  		
		  		$this->setBingID($pinResConfig[$pr]->bingKey);
		  		$properties = new Msft_Bing_MapProperties($pinResConfig[$pr]->width, $pinResConfig[$pr]->height, $pinResConfig[$pr]->zoomLevel);
		  		$this->setProperties($properties);
		  		$this->properties->setDefaultLocation($pinResConfig[$pr]->location);
		  		$this->properties->setDefaultPosition($pinResConfig[$pr]->defaultPosition);
		  	}
		  			
			$this->setconfigDetails($pinResConfig);
			return true;
	  	}
	  	else 
	  		return false;
	...
	public function savePushpins()
	{
		...
		for ($record = 0; $record < count($this->pushpinsToAdd); $record++)
		{
			...
			$pinsArray[$record]->latitude = 99.99;
			$pinsArray[$record]->longitude = 99.99;
	...
	/**
	 * Define Location entity schema and call persitenceHandler.CreateEntity() to create it
	 */
	public function createLocationEntity()
	{
		...
		$result = $this->persistenceHandler->doesEntityExist($this->bingLocation);
	
		// create table if it does not exist
		if ($result == 0)
		{
			$ent = new Msft_Bing_Entity($this->bingLocation);
			...
			$col = new Msft_Bing_Column('ARTICLE_ID');
			$col->DataType = 'int';			
			$col->AllowNull = false;
		  	$cols[] = $col;	
	 ...
	 * Call persitenceHandler.DropEntity() method to drop Configuration entity
	 */
	public function dropLocationEntity()
	{
		if ($this->persistenceHandler == null)
			throw new Msft_Bing_MapException('Persistence Handler not registered. Invoke RegisterPersistenceHandler() method');
		...
		// drop table only if it exists
		if ($result != 0) {
			$this->persistenceHandler->dropEntity($this->bingLocation);
		}
	}
	
	/**
	 * Call persitenceHandler.InsertRow to save current default location position into location entity
	 */
	public function saveDefaultPosition()
	{
		if ($this->persistenceHandler == null)
			throw new Msft_Bing_MapException('Persistence Handler not registered. Invoke RegisterPersistenceHandler() method');
		...
		$res = $this->persistenceHandler->selectRows($this->bingLocation,  $where);
	  	
	  	//Change the object structure as required to be inserterd into database
		$positionInfo = new stdClass();	
		$positionInfo->article_id = '\''.$article_id.'\'';
		$positionInfo->position = '\''.$position.'\'';
		
		if ($res != null && $res != '')
		{
			$row = new Msft_Bing_Row();
			$row->Data = $positionInfo;
			// do an update as values exist
			$whereFilter = "article_id = '$article_id'";
			$result = $this->persistenceHandler->updateRows($this->bingLocation, $row, $whereFilter);
		}
		else
		{
			$row = new Msft_Bing_Row();
			...
			//InsertRow($entityName, $row)
			$result = $this->persistenceHandler->insertRow($this->bingLocation, $row);
		}
	}
	
	public function getDefaultNodePosition($article_id)
	{			
		...
		$where = "article_id = '$article_id'";
		$rows = $this->persistenceHandler->selectRows($this->bingLocation,  $where);
		if(isset($rows[0])){	
			$item = array_change_key_case($rows[0]->Data, CASE_LOWER);			
...
class Msft_Bing_MapProperties
{
	...
	/**
	 * Latitude,Longitude position for default location of the map
	 * @var string
	 */
	...
	/**
     * Sets the latitude,longitude position for default location of the map
     * 
     * @return float
     */
	...
	/**
     * Gets the latitude,longitude position for default location of the map
     * 
     * @return float
     */
